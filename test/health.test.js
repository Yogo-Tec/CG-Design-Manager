...
test("admin can create, search and update clients", async (t) => {
  ...
  assert.equal((await fetch(`${base}/api/v1/clients/${client.id}`,{method:"DELETE",headers:{cookie}})).status,200);assert.equal((await fetch(`${base}/api/v1/clients/${client.id}`,{headers:{cookie}})).status,404);
});
...
test("admin can complete CRUD and calculations for operations",async(t)=>{const server=app.listen(0);t.after(()=>server.close());const base=`http://127.0.0.1:${server.address().port}`;const login=await fetch(`${base}/api/auth/login`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:"admin@cgdm.local",password:"admin123"})});const cookie=login.headers.get("set-cookie").split(";")[0],headers={"content-type":"application/json",cookie};const created=await fetch(`${base}/api/v1/operations/designers`,{method:"POST",headers,body:JSON.stringify({name:"CRUD Designer",status:"ACTIVE"})});assert.equal(created.status,201);const designer=(await created.json()).data.record;const updated=await fetch(`${base}/api/v1/operations/designers/${designer.id}`,{method:"PUT",headers,body:JSON.stringify({status:"INACTIVE"})});assert.equal(updated.status,200);assert.equal((await updated.json()).data.record.status,"INACTIVE");assert.equal((await fetch(`${base}/api/v1/operations/designers/${designer.id}`,{method:"DELETE",headers:{cookie}})).status,200);const project=(await(await fetch(`${base}/api/v1/operations/projects`,{method:"POST",headers,body:JSON.stringify({client_id:"00000000-0000-4000-8000-000000000010",name:"Calculation Project",status:"ACTIVE"})})).json()).data.record;const payment=(await(await fetch(`${base}/api/v1/operations/payments`,{method:"POST",headers,body:JSON.stringify({project_id:project.id,amount:250,status:"PENDING"})})).json()).data.record;const summary=await(await fetch(`${base}/api/v1/operations/reports/summary`,{headers:{cookie}})).json();assert.equal(summary.data.activeProjects,1);assert.equal(summary.data.pendingPayments,250);await fetch(`${base}/api/v1/operations/payments/${payment.id}`,{method:"DELETE",headers:{cookie}});await fetch(`${base}/api/v1/operations/projects/${project.id}`,{method:"DELETE",headers:{cookie}})});

test("reports, notifications and client approval portal respond",async(t)=>{const server=app.listen(0);t.after(()=>server.close());const base=`http://127.0.0.1:${server.address().port}`;const login=await fetch(`${base}/api/auth/login`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:"admin@cgdm.local",password:"admin123"})});const cookie=login.headers.get("set-cookie").split(";")[0];for(const path of["/api/v1/support/reports","/api/v1/support/notifications","/api/v1/files","/api/v1/operations/reports/summary"]){assert.equal((await fetch(base+path,{headers:{cookie}})).status,200)}const portal=await fetch(`${base}/api/portal/demo-client-token`);assert.equal(portal.status,200);const decision=await fetch(`${base}/api/portal/demo-client-token/decision`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({decision:"APPROVED"})});assert.equal(decision.status,200)});
